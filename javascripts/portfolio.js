document.addEventListener('DOMContentLoaded', async function() {  // Torna async
    ...
    // ===== FUNÇÕES DE SINCRONIZAÇÃO (mesmas do jornadas.js) =====
    ...
    async function buscarDadosDoBanco() {
        ...
        try {
            console.log('Buscando dados do banco para sincronizar portfólio...');
    ...
    function sincronizarTodasSemanas(dadosBanco) {
        ...
        for (let i = 1; i <= 7; i++) {
            ...
            if (dadosDaSemana) {
                ...
                console.log(`Sincronizada semana${i} para portfólio:`, dadosDaSemana);
        ...
        console.log('Carimbos atualizados para portfólio:', carimbosStatus);
    ...
    if (isUsuarioLogado()) {
        console.log('Usuário logado, sincronizando dados do banco para portfólio...');
        const dadosBanco = await buscarDadosDoBanco();
        if (dadosBanco) {
            sincronizarTodasSemanas(dadosBanco);
            console.log('✅ Todas as semanas sincronizadas com o banco para portfólio');
        } else {
            console.log('⚠️ Não foi possível buscar dados do banco, usando localStorage existente');
        }
    } else {
        console.log('Usuário não logado, usando apenas localStorage para portfólio');
    }
